...
https://github.com/CWorthy-ocean/cstar-forge and inserts their
...
# so the pre-0.1.0 history is never dumped into the release notes.
GUARD_VERSION = "0.1.0"
...
def parse_pr_body(
    ...
    *sub_items* contains any indented child bullets directly beneath it.
    ...
    def _flush() -> None:
        ...
        good = [(t, s) for t, s in collected if s or not _is_placeholder(t)]
        if good:
            result[current] = good
...
def find_unreleased_block(
    lines: list[str], latest_tag: str
) -> tuple[list[str], str]:
    """
